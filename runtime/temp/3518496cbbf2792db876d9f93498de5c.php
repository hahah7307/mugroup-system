<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:97:"/Users/hahah./Documents/wwwroot/hardware-tools/public/../application/manage/view/member/index.php";i:1607418882;s:88:"/Users/hahah./Documents/wwwroot/hardware-tools/application/manage/view/public/header.php";i:1606889687;s:86:"/Users/hahah./Documents/wwwroot/hardware-tools/application/manage/view/public/left.php";i:1607326502;s:88:"/Users/hahah./Documents/wwwroot/hardware-tools/application/manage/view/public/footer.php";i:1599103444;}*/ ?>
    ...
    <div class="right">
        ...
        <form class="layui-form search-form" method="get">
			<div class="layui-inline w80">
				<select name="sort">
					<option value="asc" <?php if($sort == 'asc'): ?>selected<?php endif; ?>>正序</option>
					<option value="desc" <?php if($sort == 'desc'): ?>selected<?php endif; ?>>倒序</option>
				</select>
			</div>
			<div class="layui-inline w150">
				<select name="rank">
					<option value="0" <?php if($rank == 0): ?>selected<?php endif; ?>>请选择职位</option>
					<?php if(is_array($ranks) || $ranks instanceof \think\Collection || $ranks instanceof \think\Paginator): if( count($ranks)==0 ) : echo "" ;else: foreach($ranks as $key=>$ran): ?>
						<option value="<?php echo $ran['id']; ?>" <?php if($rank == $ran['id']): ?>selected<?php endif; ?>><?php echo $ran['name']; ?></option>
					<?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</div>
			<div class="layui-inline w200">
				<input type="text" class="layui-input" name="keyword" value="<?php echo $keyword; ?>" placeholder="搜索企业名称">
			</div>
			<div class="layui-inline">
				<button class="layui-btn" lay-submit lay-filter="Search"><i class="layui-icon">&#xe615;</i> 查询</button>
			</div>
			<div class="layui-inline">
				<a class="layui-btn layui-btn-normal" href="<?php echo url('index'); ?>"><i class="layui-icon">&#xe621;</i> 重置</a>
			</div>
		</form>

		<div class="layui-form">
			<a class="layui-btn" href="<?php echo url('add'); ?>">添加</a>
			<table class="layui-table">
				...
				<tbody>
					<?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): if( count($list)==0 ) : echo "" ;else: foreach($list as $key=>$v): ?>
						<tr>
							<td class="tc"><a href="<?php echo $v['site_url']; ?>" target="_blank"><?php echo $v['company_name']; ?></a></td>
							<td class="tc">
								<?php if(is_array($v['member_relation']) || $v['member_relation'] instanceof \think\Collection || $v['member_relation'] instanceof \think\Paginator): if( count($v['member_relation'])==0 ) : echo "" ;else: foreach($v['member_relation'] as $key=>$rank): ?>
								<p><?php echo $rank['category']['name']; ?></p>
								<?php endforeach; endif; else: echo "" ;endif; ?>
							</td>
							<td class="tc">
								<p class="blue"><?php echo $v['legal_name']; ?></p>
								<p><?php echo $v['legal_tel']; ?></p>
								<p><?php echo $v['legal_phone']; ?>
							</td>
							<td class="tc">
								<p class="blue"><?php echo $v['lxr_name']; ?></p>
								<p><?php echo $v['lxr_post']; ?></p>
								<p><?php echo $v['lxr_tel']; ?></p>
								<p><?php echo $v['lxr_phone']; ?></p>
							</td>
							<td class="tc">
								<input type="checkbox" class="h30" name="look" value="<?php echo $v['id']; ?>" lay-skin="switch" lay-text="是|否" lay-filter="formLock" <?php if($v['status'] == 1): ?>checked<?php endif; ?>>
							</td>
							<td class="tc">
								<a href="<?php echo url('feature', ['id' => $v['id']]); ?>" class="layui-btn layui-btn-sm">风采</a>
								<a href="<?php echo url('edit', ['id' => $v['id']]); ?>" class="layui-btn layui-btn-normal layui-btn-sm">编辑</a>
								<button data-id="<?php echo $v['id']; ?>" class="layui-btn layui-btn-sm layui-btn-danger ml0" lay-submit lay-filter="Detele">删除</button>
							</td>
						</tr>
					<?php endforeach; endif; else: echo "" ;endif; ?>
				</tbody>
...
layui.use(['form', 'jquery'], function(){
	...
	form.on('submit(Sort)', function(data){
		...
		button.text('请稍候...');
		$.ajax({
			type:'POST',url:"<?php echo url('sort'); ?>",data:data.field,dataType:'json',
			success:function(data){
				if(data.code == 1){
	...
	form.on('switch(formLock)', function(data){
		$('button').attr('disabled',true);
		$.ajax({
			type:'POST',url:"<?php echo url('status'); ?>",data:{id:data.value,type:'look'},dataType:'json',
			success:function(data){
				if(data.code == 0){
					layer.alert(data.msg,{icon:2,closeBtn:0,title:false,btnAlign:'c'},function(){
						location.reload();
	...
	form.on('submit(Detele)', function(data){
		...
		layer.confirm('确定删除吗？',{icon:3,closeBtn:0,title:false,btnAlign:'c'},function(){
			...
			button.text('请稍候...');
			$.ajax({
				type:'POST',url:"<?php echo url('delete'); ?>",data:{id:id},dataType:'json',
				success:function(data){
					if(data.code == 1){
						layer.alert(data.msg,{icon:1,closeBtn:0,title:false,btnAlign:'c'},function(){
							location.reload();
