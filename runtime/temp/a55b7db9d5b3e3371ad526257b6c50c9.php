<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:101:"/Users/hahah./Documents/wwwroot/hardware-tools/public/../application/Manage/view/navigation/index.php";i:1603330983;s:88:"/Users/hahah./Documents/wwwroot/hardware-tools/application/Manage/view/public/header.php";i:1606889687;s:86:"/Users/hahah./Documents/wwwroot/hardware-tools/application/Manage/view/public/left.php";i:1607326502;s:88:"/Users/hahah./Documents/wwwroot/hardware-tools/application/Manage/view/public/footer.php";i:1599103444;}*/ ?>
				...
				<tbody>
					<?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): if( count($list)==0 ) : echo "" ;else: foreach($list as $key=>$v): ?>
						<tr>
							...
							<td class="tc">
								<input type="checkbox" class="h30" name="dropdown" value="<?php echo $v['id']; ?>" lay-skin="switch" lay-text="是|否" lay-filter="formDrop" <?php if($v['dropdown'] == 1): ?>checked<?php endif; ?>>
							</td>
							<td class="tc">
								<input type="checkbox" class="h30" name="new_target" value="<?php echo $v['id']; ?>" lay-skin="switch" lay-text="是|否" lay-filter="formTarget" <?php if($v['new_target'] == 1): ?>checked<?php endif; ?>>
							</td>
							<td class="tc">
...
layui.use(['form', 'jquery'], function(){
	...
	form.on('switch(formDrop)', function(data){
		$('button').attr('disabled',true);
		$.ajax({
			type:'POST',url:"<?php echo url('drop'); ?>",data:{id:data.value,type:'look'},dataType:'json',
			success:function(data){
				if(data.code == 0){
					layer.alert(data.msg,{icon:2,closeBtn:0,title:false,btnAlign:'c'},function(){
						location.reload();
	...
	form.on('switch(formTarget)', function(data){
		$('button').attr('disabled',true);
		$.ajax({
			type:'POST',url:"<?php echo url('target'); ?>",data:{id:data.value,type:'look'},dataType:'json',
			success:function(data){
				if(data.code == 0){
					layer.alert(data.msg,{icon:2,closeBtn:0,title:false,btnAlign:'c'},function(){
						location.reload();
